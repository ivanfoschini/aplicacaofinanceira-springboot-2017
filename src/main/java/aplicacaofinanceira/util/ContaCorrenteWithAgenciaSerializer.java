...
public class ContaCorrenteWithAgenciaSerializer extends StdSerializer<ContaCorrente> {
    ...
    public void serialize(ContaCorrente value, JsonGenerator jgen, SerializerProvider provider) throws IOException, JsonProcessingException {
        ...
        jgen.writeNumberField(Constants.CONTA_CORRENTE_ID, value.getId());
        jgen.writeNumberField(Constants.CONTA_CORRENTE_NUMERO, value.getNumero());
        jgen.writeStringField(Constants.CONTA_CORRENTE_DATA_DE_ABERTURA, DateUtil.dateToString(value.getDataDeAbertura()));
        jgen.writeNumberField(Constants.CONTA_CORRENTE_SALDO, value.getSaldo());
